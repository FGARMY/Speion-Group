...
export async function generateMetadata(
  ...
  const post = getPostBySlug(slug, 'comparisons');
  ...
  return pageMetadata({
    title: `${post.meta.title} | Speion Technology Comparisons`,
...
export async function generateStaticParams() {
  const posts = getAllPosts('comparisons');
  ...
  return (
    <article className="min-h-screen bg-slate-50 dark:bg-slate-950 pt-32 pb-24 px-4 sm:px-6 relative selection:bg-rose-500/30">
      <div className="container mx-auto max-w-3xl relative z-10">
        ...
        <Link href="/" className="inline-flex items-center text-sm font-bold text-slate-500 hover:text-rose-600 transition-colors mb-12 group">
